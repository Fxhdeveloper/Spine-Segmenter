...
RenderWindowUIMultipleInheritance::RenderWindowUIMultipleInheritance() {
  this->setupUi(this);
  QObject::connect(this->actionExit, SIGNAL(triggered()), this,
                   SLOT(slotExit()));
...
std::string RenderWindowUIMultipleInheritance::getDicomImagesFolder() {
  ...
  QString nameoffile = QFileDialog::getExistingDirectory(
      this, "Open Dicom Folder", QDir::currentPath(),
...
void RenderWindowUIMultipleInheritance::on_ReadData_clicked() {
  ...
  label_3->setText("Slice: Min = " + QString::number(itkObject->MinSlice) +
                   ", Max = " + QString::number(itkObject->MaxSlice));
  ShowCropTool();
}

void RenderWindowUIMultipleInheritance::on_UpperThreshold_valueChanged(
    int value) {
...
void RenderWindowUIMultipleInheritance::on_LowerThreshold_valueChanged(
    int value) {
...
void RenderWindowUIMultipleInheritance::on_DCM_Slice_change_sliderMoved(
    int position) {
...
void RenderWindowUIMultipleInheritance::Threshold(void) {
  ...
  itkObject->BinaryThreshold(intervertebrae, theLowerThreshold,
                             theUpperThreshold);
